...
> Template kuralı: Tier sistemi için `.claude/rules/plan-first.md` geçerlidir — Tier 3 işlerde plan zorunlu, kalıcı planlar `plans/NN-<slug>.md` formatında tutulur (`plans/feature-template.md` şablonu).
...
| Input | Mode | Behavior |
|---|---|---|
| `path/to/name.prd.md` | PRD artifact mode | Read the PRD, pick the next pending delivery milestone or implementation phase, and write `plans/{name}.plan.md` |
...
In PRD artifact mode, create `plans/` if needed. If the PRD contains a `Delivery Milestones` table, update only the selected row from `pending` to `in-progress` and set its `Plan` cell to the generated plan path.
...
When called with a `.prd.md` file, write the plan to `plans/{kebab-case-name}.plan.md` using this structure:
...
- Use `/code-check` to review the implementation
- Use `/pr` to open a pull request

## Optional Planner Agent

If a `planner` agent is available in `.claude/agents/`, you may delegate planning to it when the user explicitly asks. If the `planner` subagent is unavailable, continue planning inline instead of surfacing an "Agent type 'planner' not found" error.
